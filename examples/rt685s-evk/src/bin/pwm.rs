...
async fn main(_spawner: Spawner) {
    ...
    let ctimerlenchannel = CTimerPwmLengthChannel::new(p.CTIMER4_COUNT_CHANNEL0, MicroSeconds(10_000)).unwrap();

    // CTIMER4_MAT3 configuration for PIO0_31
    info!("GPIO0_31 is red LED on rt685-evk");
    let mut ctimer = CTimerPwm::new(p.CTIMER4_COUNT_CHANNEL3, &ctimerlenchannel, p.PIO0_31).unwrap();
